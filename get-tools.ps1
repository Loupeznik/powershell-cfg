...
$url = "https://github.com/schollz/croc/releases/download/$($version)/croc_$($version)_Windows-64bit.zip "
processDownload -application $application -url $url -extension "zip"
printResult -application $application
cleanup
...
# Get FNM
$application = "fnm"
$version = (curl -s "https://api.github.com/repos/Schniz/fnm/releases/latest" | ConvertFrom-Json).tag_name
$url = "https://github.com/Schniz/fnm/releases/download/$($version)/fnm-windows.zip "
processDownload -application $application -url $url -extension "zip"
printResult -application $application
cleanup
